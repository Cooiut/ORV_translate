...
依次输出以下两部分内容，随后告知用户"词汇加载完毕，确认后开始翻译"，然后等待确认：
...
1. 括号使用——最高频错误
   ❌ 【星座当面台词】
   ✅ ［星座当面台词］
   规则：【 】仅限真言/跨空广播；星座、鬼怪与化身的同场景普通对话一律用 ［ ］

2. 任务类别术语——必须对照词汇表，禁止自行翻译
   ❌ 副任务 / 次级任务   →  ✅ 支线任务（Sub Scenario）
   ❌ 主要任务           →  ✅ 主线任务（Main Scenario）
   ❌ 秘密任务           →  ✅ 隐藏任务（Hidden Scenario）
```
